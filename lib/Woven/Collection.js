...
var Collection = function () {
  ...
  */
	function Collection(name, schema, indexByProp) {
		...
		if (!indexByProp || !indexByProp.length) throw new Error(`Invalid indexByProp: ${indexByProp}. It should be a valid string naming the field that every entry is unique by.`);
   ...
   */
		...
		value: function insert(o) {
			if (!Collection.validateAgainstSchema(o, this)) throw new Error(`Invalid insertion item. ${o}`);
   ...
   */
		...
		value: function remove(key, value) {
			if (typeof key != "string") throw new Error(`Invalid key: ${key}. This param must be a string of the property to match value with`);
   ...
   */
		...
		key: '_fillDefaultValues',
		value: function _fillDefaultValues(entry) {
			var defaultDefinedProps = extend({}, this.propsWithDefaults);
			for (var prop in entry) {
				if (typeof entry[prop] != "undefined" && prop in defaultDefinedProps) delete defaultDefinedProps[prop];
			};
   ...
   */
		...
		key: '_get',
		value: function _get(key, value, multi) {
			if (key === this.indexingProp) {
   ...
   */
		...
		value: function validateSchema(schema) {
			...
			for (var prop in schema) {
				...
				if (typeof propDefinition != "object" || Array.isArray(propDefinition)) return false;
				if (!validTypes.has(propDefinition.type)) return false;
				if (typeof propDefinition.required !== "boolean" && typeof propDefinition.required !== "undefined") return false;
   ...
   */
		...
		value: function validateAgainstSchema(o, collectionInstance) {
			var schema = collectionInstance.schema;
			if (typeof o != "object" || Array.isArray(o)) return false;

			var requiredPropsWithNoDefaultValues = extend({}, collectionInstance.requiredWithNoDefault);
			var requiredPropsWithDefaultValues = collectionInstance.propsWithDefaults;

			for (var prop in o) {
				if (typeof o[prop] == "undefined" && prop in requiredPropsWithDefaultValues) continue;
				...
				if (isArray) {
					...
					try {
						for (var _iterator3 = propValue[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
