 ...
 */
function deepMatch() {
	for (var _i = 1; arguments[_i]; _i++) {
		var temp1 = arguments[_i - 1];
		var temp2 = arguments[_i];
...
function emptyDir(dirPath) {
	// empty dir method
	if (typeof dirPath != "string" || !fs.existsSync(dirPath)) throw new Error(`Invalid dirPath param: ${dirPath}. Either the param is not a string or a folder does not exist at specified path.`);
...
function findByProp(arr, prop, val, getIndex) {
	for (var _i2 = 0; _i2 < arr.length; _i2++) {
		if (arr[_i2][prop] == val) return getIndex ? arr[_i2] : _i2;
	};
	return getIndex && -1;
}
 ...
 */
function parseResults(matchObj) {
	...
	var toReturn = queryDoesNotExpectArray ? {} : [];
				...
				if (typeof match != "undefined") {
					...
					try {
						for (var _iterator6 = items[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
							...
							matchToReturn[_item] = match[_item];
						}
					} catch (err) {
...
function unwrap(arr) {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		if (Array.isArray(arr[i])) {
			...
			try {
				for (var _iterator7 = temp[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
